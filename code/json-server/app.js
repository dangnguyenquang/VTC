...
// Server VTC
const token =
  "dg-fDVjn2UCCp0VlJB9IIj3eeDnTud-crV9DroTKRB4OCkxC9qzqxbfH-0uor604hu6hsStS_hQUmVnmzeMfJuI4QdXbsMF88CU6PzLmt0A=";
const config = {
...
function sendMessageToAPI(data, api_url) {
  axios
    ...
    .then((response) => {
      ...
      console.log(
        data.deviceId,
        `- Đã gửi tin nhắn tới API thành công (message: ${JSON.stringify(
...
// Hàm lây giá trị pin còn lại
function getValueBattery(deviceId) {
  return BatteryPercent.findOne({ id: deviceId }) // Truy vấn dựa vào deviceId
    .then((device) => {
      if (device) {
        ...
        // Lấy thời gian bắt đầu từ bản ghi
        const startTime = new Date(device.startTime);

        // Tính số giờ chênh lệch giữa thời gian hiện tại và startTime
        const diffInHours =
          Math.abs(currentTime - startTime) / (1000 * 60 * 60);

        // Tính giá trị dựa trên số giờ chênh lệch
        let value = 100 - 10 * Math.floor(diffInHours);

        // Giảm giá trị nếu nó vượt quá giới hạn
        if (value < 0) {
          value = 0;
        }
  ...
  getIntervalWithDeviceId(deviceId)
    .then((interval) => {
      ...
      DeviceData.aggregate([
        ...
        .then((result) => {
          var now = new Date();
          const formattedDate = momentTimezome()
            .tz("Asia/Ho_Chi_Minh")
          ...
          if (result.length > 0) {
            ...
            if (
              now.getTime() - latestData.timestamp.getTime() <=
              adjustedInterval
            ) {
              state = 1;
              kind = 3;
...
async function informationAPI(deviceId) {
  const requestId = uuidv4().toUpperCase().replace(/-/g, ""); // uniqueidentifier

  try {
    const wifiInfo = await getWifiInfoById(deviceId);
    ...
    if (result.length > 0) {
      const latestData = result[0];
      const batteryPercentage = await getValueBattery(deviceId);
      ...
      const data = {
        ...
        data: {
          ...
          temperature:
            latestData.payload.data.TEM || Math.floor(Math.random() * 6) + 27,
          state: latestData.payload === "error" ? 1 : 0,
          wifi: {
            ssid_name:
              latestData.payload.data.WIF === undefined
                ? wifiInfo.wifiSSID
                : latestData.payload.data.WIF.ssid,
            password:
              latestData.payload.data.WIF === undefined
                ? wifiInfo.wifiPASS
                : latestData.payload.data.WIF.pass || "null",
            status:
              latestData.payload.data.WIF === undefined
                ? 1
                : latestData.payload.data.WIF.stt,
          },
          sim: [
            {
              number:
                latestData.payload.data.SIM === undefined
                  ? "0123556789"
                  : latestData.payload.data.SIM[0].num,
              status:
                latestData.payload.data.SIM === undefined
                  ? 1
                  : latestData.payload.data.SIM[0].stt,
            },
            // {
          ...
          battery: {
            percentage:
              latestData.payload.data.CHANGEtyp === 0 ||
              latestData.payload.data.CHANGEtyp === undefined
                ? 100
                : batteryPercentage,
            status:
              latestData.payload.data.BAT === undefined
                ? 1
                : latestData.payload.data.BAT.stt,
          },
        },
      };
...
server.post("/api/active", (req, res) => {
  ...
  if (status === 0) feeStatus = "Dang ky";
  else if (status === 1) feeStatus = "Kich hoat";
  else if (status === 2) feeStatus = "Ngung dich vu";
  else if (status === 3) feeStatus = "Huy dich vu";
  else {
...
server.get("/api/deviceinfo", (req, res) => {
  ...
  DeviceInfo.findOne({ deviceID: deviceId.substring(2, 7) })
    .then((result) => {
      ...
      if (result.feeStatus === "Dang ki") fee_status = 0;
      else if (result.feeStatus === "Kich hoat") fee_status = 1;
      else if (result.feeStatus === "Ngung dich vu") fee_status = 2;
      else if (result.feeStatus === "Huy dich vu") fee_status = 3;
      ...
      client.publish(topicToPublish, "PING", (err) => {
        if (err) {
          ...
          client.on("message", (topic, message) => {
            ...
            console.log(
              `Nhận được tin nhắn từ topic ${topic}: ${message.toString()}`
            );
...
server.post("/api/config", (req, res) => {
  ...
  deviceId =
    ...
    deviceId[2].toUpperCase() +
    deviceId.substring(3); // sẽ xóa sau khi bên VTC fix
                  ...
                  (err) => {
                    if (err) {
                      ...
                      client.on("message", (topic, message) => {
                        const strmess = message.toString();
                        console.log(
                          `Nhận được tin nhắn từ topic ${topic}: ${strmess}`
                        );
                        if (strmess.trim() === "Write WIFI SSID Successfully") {
                          clearTimeout(timerId);
                          console.log(
                            `Set thành công ssid:${data.ssid}/pass:${data.pwd}.`
                          );
                          res_message = "Thành công";
          ...
          client.publish(
            ...
            (err) => {
              if (err) {
                ...
                client.publish(
                  topicToPublish,
                  `Config Priority: ${data[0]} ${data[1]} ${data[2]}`,
                  (err) => {
                    if (err) {
                      console.error("Lỗi khi gửi tin nhắn:", err);
                      res_message = "Lỗi khi gửi tin nhắn cho mqtt broker";
                      ...
                      client.on("message", (topic, message) => {
                        ...
                        if (
                          strmess.trim().substring(0, 32) ===
                          "<!> Successfully Config Priority"
                        ) {
                          clearTimeout(timerId);
                          console.log(
                            `Set thành công Config Priority: ${data[0]} ${data[1]} ${data[2]}`
                          );
                          res_message = "Thành công";
                          success_6(data, res_message, sentResponse);
                          sentResponse = true;
